...
| Theme's brand font only shows up on some visuals (e.g. titles/slicers use it, but chart labels/axes/table text render in the Desktop default — Aptos on current builds) | Resolution (§6) picks a `visualStyles` object by **first match, whole-object** — it does not merge missing properties in from a lower-priority match for the same object name. A type-specific object (e.g. `barChart.*.labels`) that sets `fontSize`/color but omits `fontFamily` wins outright over `"*"."*".labels` or `textClasses`, and the missing `fontFamily` falls straight to system defaults, not to the wildcard/textClasses value. Add `fontFamily` explicitly inside every text-bearing object you already override (`labels`, `categoryAxis`, `valueAxis`, `legend`, `value`, `label`, `columnHeaders`, `values`, etc.) — don't rely on a wildcard or `textClasses` default reaching it |
