...
| `actionButton`/`pageNavigator`/`bookmarkNavigator` renders with blank fill and/or invisible text despite correct `fillColor`/`fontColor` hex values | The `fill`/`text` objects are missing an explicit `"show": true` property on one or both array entries — visibility silently defaults to off regardless of the color values present. This is a *second, separate* failure mode from the dual-entry-selector issue above; both must be fixed together. Validates clean, renders broken — only visible on a real Desktop/Service render. | Set explicit `"show": true` on **both** the static and `{ id: "default" }` selector entries for `fill` and `text` (and `icon` if used) — copy the dual-entry examples in [formatting.md § Dual-Entry Pattern](references/formatting.md#dual-entry-pattern) verbatim, they already include `show: true` correctly |
