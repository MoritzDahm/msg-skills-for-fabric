...
- **Confirm change-control before publishing** — if a `powerbi-governance` skill is available, invoke it to confirm a CR reference exists for the change being published, and again once the LRO reaches a terminal state so it can record the publish in the ledger.
...
**11. Log the change.** If a `powerbi-governance` skill is available,
invoke it to append/update the ledger entry — mark `Status: Applied`, list
the published item(s) with their new Fabric item ids, and note the
workspace. (Threading the CR id into `deployment-pipelines-authoring-cli`'s
deploy `note` field is a possible future extension, not built here.)
